...
function numeroMasGrande(arrayOfNums) {
   ...
   for(const i of arrayOfNums) {
      if (numeroGrande < i) numeroGrande = i
   }
   return numeroGrande
}
...
function todosIguales(array) {
   ...
   // Tu código:
   let elementoComparar = array[0]
   for (let i = 0; i < array.length; i++){
       if(array[1] !== elementoComparar) {
         return false 
       }
   }
   return true 
...
function tablaDelSeis() {
   ...
   // Tu código:
   let arrayTabalDelSeis =[];
   for(let i = 0; i <= 10; i++) {
      arrayTabalDelSeis.push (6 * i);
   }
   return arrayTabalDelSeis;
}
...
function mayorACien(array) {
   ...
   let nuevoArrayEnteros = [];
   for(let i = 0; i < array.length; i++) {
      if (array[i] > 100) {
         nuevoArrayEnteros.push (array[i]);
      } 
   }return nuevoArrayEnteros
}
